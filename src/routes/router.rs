...
use crate::routes::cache_remote::RemoteCache;
...
/// 匹配所有其他路径
pub async fn cache_router(req: HttpRequest, body: web::Bytes) -> impl Responder {
...
async fn handle_get(path: &str) -> Result<HttpResponse, std::io::Error> {
    ...
    if !local.exist() {
        let remote = RemoteCache::new(path.to_string(), local.find());
        if remote.exist().await.unwrap() {
            remote.read().await.unwrap();
        }
    }
...
async fn handle_put(path: &str, data: &[u8]) -> Result<HttpResponse, std::io::Error> {
    ...
    match local.write(data) {
        Ok(_) => {
            let remote = RemoteCache::new(path.to_string(), local.find());
            remote.write().await.unwrap();

            Ok(HttpResponse::Ok().body("Write file successfully"))
        }
